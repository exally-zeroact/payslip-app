 ...
 * 【乙欄】令和8の乙欄は本実装では未収録(率の一次情報を機械抽出で確定できず)→ otsuManual=true を返し手入力に委ねる(捏造しない)。
 * 【利用】ブラウザ window.ShoyoZei / Node require('./shoyo-zei.js')
 * ================================================================ */
(function (root, factory) {
  if (typeof module !== "undefined" && module.exports) { module.exports = factory(); }
  else { root.ShoyoZei = factory(); }
  ...
  // 賞与の率(%)。甲欄のみ。乙(taxClass==='otsu')は {otsuManual:true}。範囲外年は直近年+stale。
  function bonusRate(prevAfterSI, fuyou, opts) {
    opts = opts || {};
    if (opts.taxClass === "otsu") return { otsu: true };
  ...
  function calcBonusTax(o) {
    ...
    if (o.taxClass === "otsu") return { tax: 0, otsu: true, prevAfter: prevAfter };
    // 特例: 前月給与なし/社保以下、または賞与(社保後)が前月課税ベースの10倍超 → 月額表で計算(本表によらない)
    if (prevAfter <= 0 || taxableBonus > prevAfter * 10) return { tax: 0, special: true, prevAfter: prevAfter };
    var r = bonusRate(prevAfter, o.fuyou, { taxClass: "ko", year: year });
    var tax = Math.floor(taxableBonus * r.rate / 100); // 円未満切捨
    return { tax: tax, rate: r.rate, special: false, otsu: false, prevAfter: prevAfter, year: r.year, stale: r.stale };
  }
  ...
  return {
    ...
    calcBonusTax: calcBonusTax, calcBonusSI: calcBonusSI,
    RATES: RATES, KOU_BY_YEAR: KOU_BY_YEAR, KENPO_YEAR_CAP: KENPO_YEAR_CAP, KOSEI_PER_CAP: KOSEI_PER_CAP
  };
});
