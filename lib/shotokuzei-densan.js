...
(function (root, factory) {
  ...
  function hydrate(year, data) {
    ...
    if (typeof data.fuyouKojo === 'number') p.fuyouKojo = data.fuyouKojo; // calcは paramsFor(r8).fuyouKojo を読む(export FUYOU_KOJOは既定値の参照専用)
    if (Array.isArray(data.kyuyo) && data.kyuyo.length) p.kyuyo = normUpto(data.kyuyo);
    if (Array.isArray(data.kiso) && data.kiso.length) p.kiso = normUpto(data.kiso);
    // 甲乙税率表は in-place で差替=export参照(ZEI_KO/ZEI_OTSU)も追従・evalTaxも同一配列を読む(stale export防止)
    if (Array.isArray(data.zeiKo) && data.zeiKo.length) { var nk = normUpto(data.zeiKo); ZEI_KO.length = 0; for (var i = 0; i < nk.length; i++) ZEI_KO.push(nk[i]); }
    if (Array.isArray(data.zeiOtsu) && data.zeiOtsu.length) { var no = normUpto(data.zeiOtsu); ZEI_OTSU.length = 0; for (var j = 0; j < no.length; j++) ZEI_OTSU.push(no[j]); }
  }
