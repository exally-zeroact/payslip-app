...
(function (root, factory) {
  ...
  function zuijiKaitei(opts) {
    ...
    var ng = (SH && SH.gradeOf) ? SH.gradeOf(avg) : null;
    var pg = (SH && SH.gradeOf) ? SH.gradeOf(num(opts.prevHyojun)) : null;
    var diff = (ng && pg) ? Math.abs(ng.tokyu - pg.tokyu) : 0;
    var fixedChanged = !!opts.fixedChanged;
    var eligible = fixedChanged && allDays && diff >= 2 && num(opts.prevHyojun) > 0;
    var reasons = [];
    ...
    if (num(opts.prevHyojun) <= 0) reasons.push('従前の標準報酬月額の入力が必要');
    else if (diff < 2) reasons.push('2等級以上の差が必要（現在' + diff + '等級差）');
    // 適用月=変動月の4か月目
    var apply = '';
    ...
    return { eligible: eligible, newHyojun: ng ? ng.hyojun : avg, newGrade: ng ? ng.tokyu : 0, prevGrade: pg ? pg.tokyu : 0, gradeDiff: diff, allDays: allDays, avg: avg, applyYm: apply, reasons: reasons };
  }
