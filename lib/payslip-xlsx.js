...
(function (root, factory) {
  ...
  function shukeiAOA(people, opts){
    ...
    aoa.push(['氏名','支給合計','控除合計','差引支給額']);
    var ts=0,tk=0,tn=0;
    people.forEach(function(p){ var s=p.shikyuTotal!=null?num(p.shikyuTotal):0, k=p.kojoTotal!=null?num(p.kojoTotal):0, n=num(p.net);
      ts+=s; tk+=k; tn+=n; aoa.push([p.name||'', s, k, n]); });
    aoa.push(['合計', ts, tk, tn]);
    return { aoa: aoa, cols: [{wch:18},{wch:14},{wch:14},{wch:14}],
      merges: [{s:{r:0,c:0},e:{r:0,c:3}},{s:{r:1,c:0},e:{r:1,c:3}}] };
  }
