...
(function (root, factory) {
  ...
  var P = {
    ...
    // 基礎控除(令和8・9年分) 合計所得金額別: ≤489万→104万 / 〜655→67 / 〜2350→62 / 超は86条48/32/16/0。
    kisoKojo: [
      { upto: 4890000, flat: 1040000 }, { upto: 6550000, flat: 670000 }, { upto: 23500000, flat: 620000 },
  ...
  function hydrate(year, data) {
    ...
    if (Array.isArray(data.kyuyoKojo) && data.kyuyoKojo.length) P.kyuyoKojo = normUpto(data.kyuyoKojo);
    if (Array.isArray(data.kisoKojo) && data.kisoKojo.length) P.kisoKojo = normUpto(data.kisoKojo);
    if (data.fuyoKojo && typeof data.fuyoKojo === 'object') P.fuyoKojo = data.fuyoKojo;
    if (data.haiguusha && Array.isArray(data.haiguusha.normal) && Array.isArray(data.haiguusha.rojin)) P.haiguusha = data.haiguusha;
    if (Array.isArray(data.haiTokubetsu) && data.haiTokubetsu.length) P.haiTokubetsu = normUpto(data.haiTokubetsu);
    if (Array.isArray(data.tokuteiShinzoku) && data.tokuteiShinzoku.length) P.tokuteiShinzoku = normUpto(data.tokuteiShinzoku);
    if (data.shougai && typeof data.shougai === 'object') P.shougai = data.shougai;
    if (Array.isArray(data.sanshutu) && data.sanshutu.length) P.sanshutu = normUpto(data.sanshutu);
    if (typeof data.fukkoRate === 'number' && data.fukkoRate > 0) P.fukkoRate = data.fukkoRate;
  }
