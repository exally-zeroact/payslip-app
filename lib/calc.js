...
(function (root, factory) {
  ...
  function taxableTotal(shikyu) {
    return (shikyu || []).reduce(function (a, it) {
      var v = num(it.value);
      var hikazei = !!it.hikazei || NONTAX_RE.test(it.label || '');
  ...
  function computePayslip(emp) {
    ...
    var apply = emp.apply || {};
    var on = function (k) { return apply[k] !== false && (k === 'health' ? hasHealth : k === 'pension' ? hasPension : true); };
    var si = PayrollCalc.calcSocialInsurance({ payTotal: payTotal, hyojunBase: num(emp.hyojunBase), healthRate: emp.healthRate, pref: emp.pref, employRate: emp.employRate, hasKaigo: hasKaigo, payYm: emp.payYm });
    // 年齢資格喪失(厚年70/健保75)は si 自体を0に=控除/課税/手取りだけでなく 社保サマリー/一覧/賃金台帳/保存 まで一致させる。
    if (!hasPension) si.pension = 0;
    if (!hasHealth) { si.health = 0; si.kaigo = 0; }
    if (!hasPension || !hasHealth) si.total = num(si.health) + num(si.pension) + num(si.kaigo) + num(si.employ);
