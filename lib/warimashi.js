...
(function (root, factory) {
  ...
  // 全率（詳細モードの排他7区分）法定下限の既定。会社/従業員でrates上書き可。
  var RATE = { ot: 1.25, otNight: 1.5, over60: 1.5, over60Night: 1.75, night: 0.25, holiday: 1.35, holidayNight: 1.6 };
  // 率オーバーライド解決：基本率(ot/holiday)＋増分(night/over60Add)。未指定は法定下限。会社は上げられる。
  function resolveRates(rates) {
    var r = rates || {};
    return {
      ot: r.ot != null && r.ot !== '' ? num(r.ot) : 1.25,
      holiday: r.holiday != null && r.holiday !== '' ? num(r.holiday) : 1.35,
      night: r.night != null && r.night !== '' ? num(r.night) : 0.25,
      over60Add: r.over60Add != null && r.over60Add !== '' ? num(r.over60Add) : 0.25
    };
  }
  ...
  function easyComponents(otMin, nightMin, holidayMin, rates) {
    ...
    var c = [];
    if (otMin > 0) c.push({ key: 'ot', label: '残業（時間外）', rate: R.ot, minutes: otMin });
  ...
  function detail(opts) {
    ...
    return calc(unit, detailComponents(opts.seg || {}, opts.rates));
  }

  // かんたん総合: 入力(時間/分)→ 割増合計。opts.rates で率上書き可。
  function easy(opts) {
    var unit = hourlyUnit(opts.base, monthlyStdHours(opts.annualHolidays, opts.dailyHours, opts.leap));
    var comps = easyComponents(hm2min(opts.otH, opts.otM), hm2min(opts.nightH, opts.nightM), hm2min(opts.holidayH, opts.holidayM), opts.rates);
  ...
  return {
    ...
    RATE: RATE, resolveRates: resolveRates, calc: calc, easyComponents: easyComponents, easy: easy,
