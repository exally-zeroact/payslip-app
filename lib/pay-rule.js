...
//  part = { type:'hourly'|'daily'|'commission'|'rate'|'fixed', amount:number, label?:string }
...
//  ★spec=構造+固定レート(時給/日給/率%/固定額)。ctx=当月データ(歩合額/売上/労働時間/出勤日数)。
//  ctx = { workMin:0, workDays:0, sales:0, commission:0 }
//
...
(function (root, factory) {
  ...
  function partLabel(part) {
    ...
    switch (part && part.type) {
      case 'hourly': return '時給×時間'; case 'daily': return '日給×日数';
      case 'commission': return '歩合'; case 'rate': return '売上×率'; case 'fixed': return '固定';
      default: return '—';
    }
  }
  ...
  function basePay(spec, ctx) {
    ...
    var isPiecework = (chosenType === 'commission' || chosenType === 'rate');
    return {
      base: base,
      fixed: fixed,
      variable: vr.amount,
  ...
  return { evalPart: evalPart, evalVariable: evalVariable, basePay: basePay, partLabel: partLabel };
});
