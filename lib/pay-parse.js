...
(function (root, factory) {
  ...
  function parse(text) {
    ...
    // 売上×率: "売上の3.5割" "売上35%" or 単独の "3.5割/35%"
    var mRate = grab(/売上[のを×xX*]?\s*([\d.]+\s*(?:割(?:[\d.]+分)?|%))/) || grab(/([\d.]+\s*(?:割(?:[\d.]+分)?|%))/);
    // 歩合(語のみ・額は毎月入力)
    var mCom = /歩合|出来高|インセン/.test(s);

    if (mHr) parts.push({ type: 'hourly', amount: yen(mHr[1]) });
    if (mDay && fixedAmt == null) { /* 日給は下でsimple判定 */ }
    if (mPiece) parts.push({ type: 'piece', amount: yen(mPiece[1]) });
    if (mRate) parts.push({ type: 'rate', amount: pct(mRate[1]) });
    if (mCom && !mRate && !mPiece) parts.push({ type: 'commission' }); // 「歩合」だけ=歩合額を毎月入力
    ...
    // 変動部品ゼロ = 単純形(月給/時給/日給)
    if (!parts.length) {
      if (mDay) return finalizeSimple('日給', yen(mDay[1]), s, used);
    ...
    // 時給のみ・固定なし = 時給制
    if (parts.length === 1 && parts[0].type === 'hourly' && fixedAmt == null) {
      return finalizeSimple('時給', parts[0].amount, s, used);
    }
    // それ以外 = カスタム(固定 + 変動{one/max})
    var mode = (parts.length >= 2 && isMax) ? 'max' : (parts.length >= 2 ? 'max' : 'one');
    var payRule = { fixed: fixedAmt != null ? String(fixedAmt) : '', variable: { mode: mode, parts: parts.map(function (p) { return { type: p.type, amount: p.amount != null ? String(p.amount) : '', label: '' }; }) } };
    var sum = summaryOf(fixedAmt, mode, parts);
    return { ok: true, payType: 'カスタム', fields: { payRule: payRule }, summary: sum, unrecognized: leftover(text, used), low: false };
  }
  ...
  function leftover(text, used) {
    var s = String(text || ''); used.forEach(function (u) { s = s.replace(u, ''); });
    s = s.replace(/[のをか＋+とやまたはいずれか高い方いい方良い方大きい方どちらか、。・\s　]/g, '');
    return s;
  }
