...
T('★健康保険 東京 令和8年度: 9.85% ＋ 子育て支援金 0.23% = 10.08%（折半 5.04%）', function () {
  const p = one({}, '2026-06');
  near(p.si.health * 2 / p.hyojunHealth, 0.1008, 0.0006, '東京健保+支援金(折半前)');
});
T('介護保険 令和8年度: 1.62%（全国一律・40〜64歳のみ）', function () {
  const y = one({ birthYmd: '1981-02-20' }, '2026-06');   // 45歳=対象
  const n = one({ birthYmd: '1990-01-01' }, '2026-06');   // 36歳=対象外
  near(y.si.kaigo * 2 / y.hyojunHealth, 0.0162, 0.0004, '介護(折半前)');
